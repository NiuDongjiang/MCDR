    ...
    def forward(self, graph, feat, weight=None, Two_Stage=False):
        ...
        with graph.local_scope():
            if isinstance(feat, tuple):
                feat, _ = feat#313,3|593,3  # dst feature not used [drug or disease num , 3]
            cj = graph.srcdata['cj']#313, 1
            ci = graph.dstdata['ci']#593, 1
            ...
            if weight is not None:
                feat = dot_or_identity(feat, weight, self.device)#313,840

            feat = feat * self.dropout(cj)
            graph.srcdata['h'] = feat
            graph.update_all(fn.copy_src(src='h', out='m'),
                             fn.sum(msg='m', out='h'))
            rst = graph.dstdata['h']#593,840
            rst = rst * ci
...
class GCMCLayer(nn.Module):
    ...
    def forward(self, graph, drug_feat=None, dis_feat=None, Two_Stage=False):
        ...
        drug_feat = out_feats['drug']#593,840
        dis_feat = out_feats['disease']#313,840
...
class Net(nn.Module):
    ...
    def forward(self, enc_graph, dec_graph,
        ...
        ddi_out, dis_sim_out = self.FGCN(ddi_graph, ddi_feat, dis_graph, disease_sim_feat)
        drug_out, dis_out = None, None
        #drug_feat, dis_feat = drug_sim_out, dis_sim_out
        for i in range(0, self.layers):#593,3
            drug_o, dis_o = self.TGCN[i](enc_graph, drug_feat, dis_feat, Two_Stage)  # 593,75|313,75
            if i == 0:
                drug_o = drug_o * ddi_out
                dis_o = dis_o * dis_sim_out
                drug_out = drug_o
                dis_out = dis_o
            else:
        ...
        return pred_ratings, drug_out, ddi_out, dis_out, dis_sim_out
