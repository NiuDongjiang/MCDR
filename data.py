...
class DrugNovoLoader(object):
    ...
    def _generate_feat_graph(self):
        ...
        # ddi graph
        if drug_num_neighbor > drug_ddi.shape[0] or drug_num_neighbor < 0:
            drug_num_neighbor = drug_ddi.shape[0]

        drug_neighbor = np.argpartition(-drug_ddi, kth=drug_num_neighbor, axis=1)[:, :drug_num_neighbor]
        ddi_row_index = np.arange(drug_neighbor.shape[0]).repeat(drug_neighbor.shape[1])
        ddi_col_index = drug_neighbor.reshape(-1)
        ddi_edge_index = np.array([ddi_row_index, ddi_col_index]).astype(int).T

        ddi_edges = np.array(list(ddi_edge_index), dtype=np.int32).reshape(ddi_edge_index.shape)
        ddi_adj = sp.coo_matrix((np.ones(ddi_edges.shape[0]), (ddi_edges[:, 0], ddi_edges[:, 1])),
                                shape=(self.num_drug, self.num_drug),
                                dtype=np.float32)
        ddi_adj = ddi_adj + ddi_adj.T.multiply(ddi_adj.T > ddi_adj) - ddi_adj.multiply(
            ddi_adj.T > ddi_adj)
        # drug_graph = normalize(drug_adj + sp.eye(drug_adj.shape[0]))
        ddi_graph = normalize(ddi_adj)
        ddi_graph = sparse_mx_to_torch_sparse_tensor(ddi_graph)
        ...
        return drug_graph, disease_graph, ddi_graph
...
class DrugDataLoader(object):
    ...
    def _load_drug_data(self, file_path, data_name):
        ...
        if data_name in ['Gdataset']:
            ...
            self.disease_sim_features = data['disease']
            self.drug_sim_features = data['drug']
            self.ddi = pd.read_csv('./G_ddi_prediction_matrix.csv', index_col=0).values
        elif data_name in ['Cdataset']:
            data = sio.loadmat(file_path)
            association_matrix = data['didr'].T
            self.disease_sim_features = data['disease']
            self.drug_sim_features = data['drug']
            self.ddi = pd.read_csv('./C_ddi_prediction_matrix.csv', index_col=0).values
            ...
            self.ddi = pd.read_csv('./L_ddi_prediction_matrix.csv',index_col=0).values
        elif data_name in ['HDVD']:
            path = os.path.join('./virusdrug.mat')
            data = io.loadmat(path)
            association_matrix = data['virusdrug']
            path_disease = os.path.join('./virussim.mat')
            path_drug = os.path.join('./drugsim.mat')
            self.disease_sim_features = io.loadmat(path_disease)['virussim']
            self.drug_sim_features = io.loadmat(path_drug)['drugsim']
            self.ddi = pd.read_csv('./HDVD_ddi_prediction_matrix.csv', index_col=0).values
        ...
        for (train_pos_idx, test_pos_idx), (train_neg_idx, test_neg_idx) in zip(kfold.split(pos_row),
            ...
            #test_neg_edge = np.stack([neg_row[test_neg_idx][0:10*len(test_pos_values)], neg_col[test_neg_idx][0:10*len(test_pos_values)]])
