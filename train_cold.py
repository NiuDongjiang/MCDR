...
if __name__ == '__main__':
    ...
    args.save_dir = os.path.join("disease_denovo", args.save_dir)
    ...
    print("Mean_AUROC{:4f}".format(np.mean(auc_list)), "std{:4f}".format(np.std(auc_list)), "Mean_AURP{:4f}".format(np.mean(aupr_list)), "std{:4f}".format(np.std(aupr_list)))
